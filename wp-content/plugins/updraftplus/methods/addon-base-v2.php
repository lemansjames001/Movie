...
abstract class UpdraftPlus_RemoteStorage_Addons_Base_v2 extends UpdraftPlus_BackupModule {
	...
	public function delete_files($ret, $files, $ignore_it = false) {// phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		if ($this->supports_feature('multi_delete')) {
			$updraftplus->log($this->method.": Delete remote files: ".implode($files));
			try {
				...
				if (is_array($responses)) {
					foreach ($responses as $key => $response) {
						if ('success' == $response) {
							$updraftplus->log($this->method.": $files[$key]: Delete succeeded");
						} elseif (is_array($response)) {
							$ret = false;
							if (isset($response['error']) && isset($response['error']['code']) && isset($response['error']['message'])) {
								$updraftplus->log($this->method.": Delete failed for file: $files[$key] with error code: ".$response['error']['code']." message: ".$response['error']['message']);
							} else {
								$updraftplus->log($this->method.": Delete failed for file: $files[$key]");
							}
						}
					}
				} elseif (!$responses) {
					$ret = false;
					$updraftplus->log($this->method.": Delete failed for files: ".implode($files));
				}
			} catch (Exception $e) {
				$updraftplus->log('ERROR: '.$this->method.": ".implode($files).": Failed to delete files: ".$e->getMessage().' (code: '.$e->getCode().', line: '.$e->getLine().', file: '.$e->getFile().')');
				$ret = false;
			}
